(async function () {
  const response = await fetch('/site-base.html?v=20260829', { cache: 'no-store' });
  if (!response.ok) throw new Error('Não foi possível carregar a página-base.');
  let s = await response.text();

  const newProcess = `  <!-- Destaque processo judicial -->
  ...
  s = s.replace(/\s*<!-- Destaque processo judicial -->[\s\S]*?(?=\s*<!-- Documentos do processo -->)/, '\n' + newProcess + '\n');
  s = s.replace(
  ...
  const newDocsDownload = `      <div class="docs-download">
  ...
  s = s.replace(/\s*<div class="docs-download">[\s\S]*?<\/div>/, '\n' + newDocsDownload);

  const coverageCSS = `
    /* ── Repercussão na imprensa ── */
    .media-subtitle{font-size:14px;font-weight:800;color:var(--text);margin:28px 0 12px}
    .coverage-grid{display:grid;grid-template-columns:1fr;gap:12px}
    .coverage-card{display:block;background:var(--bg);border:1px solid #e5e7eb;border-radius:10px;padding:14px 16px;text-decoration:none;color:var(--text);transition:border-color .15s,transform .1s,box-shadow .15s}
    .coverage-card:hover{border-color:var(--accent);transform:translateY(-1px);box-shadow:var(--shadow)}
    .coverage-top{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:5px}
    .coverage-outlet{font-size:14px;font-weight:800}
    .coverage-kind{font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;color:var(--accent);background:#fee2e2;border-radius:999px;padding:3px 7px;white-space:nowrap}
    .coverage-card h3{font-size:.92rem;line-height:1.35;margin-bottom:4px}
    .coverage-card p{font-size:12px;line-height:1.45;color:var(--muted)}
    .coverage-note{font-size:12px;color:#64748b;margin-top:14px;line-height:1.5}
    .video-links-grid{display:grid;grid-template-columns:1fr;gap:12px;margin-top:12px}
    .video-source-card{display:flex;align-items:center;gap:12px;background:#111827;color:#fff;text-decoration:none;border-radius:10px;padding:14px 16px;transition:transform .1s,opacity .15s}
    .video-source-card:hover{transform:translateY(-1px);opacity:.94}
    .video-source-icon{display:flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:50%;background:var(--accent);font-size:16px;flex:0 0 auto}
    .video-source-card strong{display:block;font-size:14px;line-height:1.3}
    .video-source-card span span{display:block;font-size:12px;color:#d1d5db;margin-top:2px}
    @media(min-width:600px){.coverage-grid,.video-links-grid{grid-template-columns:repeat(2,1fr)}}
`;
  if (!s.includes('/* ── Repercussão na imprensa ── */')) {
    s = s.replace('    /* ── Linha do tempo ── */', coverageCSS + '    /* ── Linha do tempo ── */');
  }
  ...
  const cards = coverage.map(([outlet,kind,title,desc,url]) =>
    `<a class="coverage-card" href="${esc(url)}" target="_blank" rel="noopener noreferrer"><div class="coverage-top"><span class="coverage-outlet">${esc(outlet)}</span><span class="coverage-kind">${esc(kind)}</span></div><h3>${esc(title)}</h3><p>${esc(desc)}</p></a>`
  ).join('\n');

  const videoSection = `  <!-- Vídeos -->
  <section id="videos">
    <div class="container">
      ...
      <div class="video-grid"><div class="video-card"><div class="video-embed"><iframe src="https://www.youtube.com/embed/DYK_Ft74V9g" title="Reportagem Sucesso no Campo — Operação Simulatio" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen loading="lazy"></iframe></div><div class="video-info"><span class="video-tag">Sucesso no Campo · 12/08/2026</span><h3>Fraude em licitação — reportagem sobre a Operação Simulatio</h3><p>Reportagem com imagens da operação em Rio Verde e trechos da coletiva da Prefeitura.</p></div></div></div>
  ...
  s = s.replace(/\s*<!-- Vídeos -->[\s\S]*?(?=\s*<!-- Linha do tempo simplificada -->)/, '\n' + videoSection + '\n');
  ...
  if (s.includes('11rPvp1Qwe0jn6RVDJICu00kThPRfnTQ-') || s.includes('enquanto ainda é público') || s.includes('não está em segredo de justiça')) {
    throw new Error('A página-base ainda contém texto de acesso antigo após a transformação.');
  }
  ...
  console.error(err);
  document.body.innerHTML = '<main style="max-width:720px;margin:60px auto;padding:20px;font:16px system-ui"><h1>Não foi possível carregar a página.</h1><p>Tente atualizar. Se o problema persistir, acesse a versão-base.</p><p><a href="/site-base.html">Abrir versão-base</a></p></main>';
});
